...
DATAPATH .................... ~92% ✅ OPERAND2 + CPSR FLAG STORAGE
  ...
  condition check .................. NEXT (cond[4] × NZCV → condition_pass)
...
The 4-bit register stores `CPSR[3:0]=NZCV`. `instr[20]` (S) is its current enable;
the future final enable is `S AND condition_pass`. Arithmetic regression ROM:
...
Next: condition check, branch/BX decoder extension, memory, then shifter carry cleanup.
...
- **armv4t.circ (V2) now contains 29 circuits:** `main`, `koggle_stone_1b`,
...
**PROJECT PAUSED 2026-07-27. FIRST TASK WHEN RESUMING: BUILD `condition_checker`.**
Do not start branch, memory, or shifter-carry work first. Wire
`CPSR.N/Z/C/V + instr[31:28] -> condition_checker -> condition_pass`, then gate
both `reg16x32.WE` and `CPSR.enable` with `condition_pass`. Use the canonical
ARMv4 condition table already recorded below in step 5.
  ...
  5. **CONDITION CHECK** ← START HERE
     `instr[31:28] + CPSR.NZCV -> condition_pass`; failed conditions must suppress
     register writes and CPSR updates.
  6. B + BX control flow
  7. LDR / STR decoder extension + data RAM
  8. BL link writeback, stack/ABI cleanup, and shifter-carry correctness
...
toward full single-cycle CPU (runs C) ~92%. Must-BUILD: condition check,
branch/BX, decoder extension, memory, and ABI support. Shifter carry remains a
correctness cleanup.
