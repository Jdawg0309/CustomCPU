...
R = 512                      # result area, word 0x80
def w(byte_addr): return byte_addr // 4
...
T("nested_lifo", [
    "    mov sp, #256", "    mov r0, #17", "    mov r1, #34", "    push {r0,r1}",
    "    mov r2, #51", "    mov r3, #68", "    push {r2,r3}",
    "    pop {r4,r5}", "    pop {r6,r7}", "    mov r11, #512",
    "    str r4, [r11]", "    str r5, [r11, #4]",
    "    str r6, [r11, #8]", "    str r7, [r11, #12]", "    str r13, [r11, #16]"],
  {w(R):0x33, w(R+4):0x44, w(R+8):0x11, w(R+12):0x22, w(R+16):0x100},
  allow_extra=[w(0xf8), w(0xfc), w(0xf0), w(0xf4)])

# --- ordinary loads/stores interleaved with a live stack frame ----------
T("interleave_mem", [
    "    mov sp, #256", "    mov r0, #170", "    push {r0}",
    "    mov r1, #128", "    mov r2, #85", "    str r2, [r1]", "    ldr r3, [r1]",
    "    pop {r4}", "    mov r11, #512",
    "    str r3, [r11]", "    str r4, [r11, #4]", "    str r13, [r11, #8]"],
  {w(R):0x55, w(R+4):0xAA, w(R+8):0x100, w(128):0x55},
  allow_extra=[w(0xfc)])

# --- three sequential single pushes, then three pops --------------------
T("triple_sequential", [
    "    mov sp, #256", "    mov r0, #1", "    push {r0}",
    "    mov r0, #2", "    push {r0}", "    mov r0, #3", "    push {r0}",
    "    pop {r1}", "    pop {r2}", "    pop {r3}", "    mov r11, #512",
    "    str r1, [r11]", "    str r2, [r11, #4]",
    "    str r3, [r11, #8]", "    str r13, [r11, #12]"],
  {w(R):3, w(R+4):2, w(R+8):1, w(R+12):0x100},
  allow_extra=[w(0xfc), w(0xf8), w(0xf4)])

# --- registers NOT in the list must survive untouched -------------------
T("preserves_others", [
    "    mov sp, #256", "    mov r5, #90", "    mov r0, #1", "    mov r1, #2",
    "    push {r0,r1}", "    pop {r2,r3}", "    mov r11, #512",
    "    str r5, [r11]", "    str r2, [r11, #4]",
    "    str r3, [r11, #8]", "    str r13, [r11, #12]"],
  {w(R):0x5A, w(R+4):1, w(R+8):2, w(R+12):0x100},
  allow_extra=[w(0xf8), w(0xfc)])

# --- function prologue/epilogue shape: callee-saved + LR -----------------
T("lr_call_pattern", [
    "    mov sp, #256", "    mov r14, #153", "    mov r4, #68",
    "    push {r4,r14}", "    mov r4, #0", "    mov r14, #0",
    "    pop {r4,r14}", "    mov r11, #512",
    "    str r4, [r11]", "    str r14, [r11, #4]", "    str r13, [r11, #8]"],
  {w(R):0x44, w(R+4):0x99, w(R+8):0x100},
  allow_extra=[w(0xf8), w(0xfc)])

# --- boundary values through the whole path -----------------------------
T("zero_and_all_ones", [
    "    mov sp, #256", "    mov r0, #0", "    mvn r1, #0",
    "    push {r0,r1}", "    pop {r2,r3}", "    mov r11, #512",
    "    str r2, [r11]", "    str r3, [r11, #4]", "    str r13, [r11, #8]"],
  {w(R+4):0xFFFFFFFF, w(R+8):0x100},          # r2==0 is omitted by the dumper
  allow_extra=[w(0xf8), w(0xfc)])

# --- stack at the very top of RAM ---------------------------------------
T("stack_at_ram_top", [
    "    mov sp, #1024", "    mov r0, #119", "    mov r14, #187",
    "    push {r0,r14}", "    pop {r1,r2}", "    mov r11, #512",
    "    str r1, [r11]", "    str r2, [r11, #4]", "    str r13, [r11, #8]"],
  {w(R):0x77, w(R+4):0xBB, w(R+8):0x400},
  allow_extra=[w(0x3f8), w(0x3fc)])

# --- high registers only -------------------------------------------------
T("high_regs_only", [
    "    mov sp, #256", "    mov r8, #136", "    mov r9, #153",
    "    mov r10, #160", "    push {r8,r9,r10}", "    pop {r5,r6,r7}",
    "    mov r11, #512", "    str r5, [r11]", "    str r6, [r11, #4]",
    "    str r7, [r11, #8]", "    str r13, [r11, #12]"],
  {w(R):0x88, w(R+4):0x99, w(R+8):0xA0, w(R+12):0x100},
  allow_extra=[w(0xf4), w(0xf8), w(0xfc)])
